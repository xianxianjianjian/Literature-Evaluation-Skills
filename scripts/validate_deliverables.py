#!/usr/bin/env python3
"""Validate V1 repository structure, workflow state, and A/B/C contracts.

The validator distinguishes academic workflow completion from optional Zotero
archive closure. Structural checks never replace visual QA or academic judgment.
"""
...
def check_foundation(root: Path) -> list[Check]:
    ...
    for name, expected in CSV_HEADERS.items():
        ...
        try:
            ...
            checks.append(Check(f"schema:{name}", actual == expected, "header valid" if actual == expected else "header mismatch"))
...
def _academic_completion_checks(data: dict) -> list[Check]:
    ...
    checks.append(Check("academic:paper-id", paper_ok, "paper_id present" if paper_ok else "academic workflow requires paper_id"))
    no_updates = not any(stage["needs_update"] for stage in stages.values())
    checks.append(Check("academic:no-needs-update", no_updates, "no unresolved needs_update" if no_updates else "academic workflow has unresolved needs_update"))
    no_blockers = not data.get("blocking_issues")
    checks.append(Check("academic:no-blockers", no_blockers, "no academic blockers" if no_blockers else "academic workflow has unresolved blocking_issues"))
    source_checked = bool(data.get("source_change", {}).get("last_checked"))
    checks.append(Check("academic:source-check", source_checked, "source-change check dated" if source_checked else "academic workflow requires source_change.last_checked"))
    return checks
...
def check_manifest(
    path: Path,
    *,
    require_workflow_complete: bool = False,
    require_academic_complete: bool = False,
    require_archive_complete: bool = False,
) -> list[Check]:
    """Validate manifest relationships and optional completion levels.

    `require_workflow_complete` is retained as a backwards-compatible alias for
    strict archive completion. New V1 callers should choose academic or archive
    ...
    if stages["translation"]["status"] == "COMPLETE":
        ...
        checks.append(Check("manifest:translation-output", passed, "A COMPLETE" if passed else "Translation COMPLETE requires A COMPLETE"))
    if outputs["A"]["status"] == "COMPLETE":
        passed = stages["translation"]["status"] == "COMPLETE"
        checks.append(Check("manifest:A-translation", passed, "Translation COMPLETE" if passed else "A COMPLETE requires Translation COMPLETE"))

    if stages["deep_reading"]["status"] == "COMPLETE":
        passed = outputs["B"]["status"] == "COMPLETE"
        checks.append(Check("manifest:deep-reading-B", passed, "B COMPLETE" if passed else "Deep Reading COMPLETE requires B COMPLETE"))
    if outputs["B"]["status"] == "COMPLETE":
        passed = stages["deep_reading"]["status"] == "COMPLETE"
        checks.append(Check("manifest:B-deep-reading", passed, "Deep Reading COMPLETE" if passed else "B COMPLETE requires Deep Reading COMPLETE"))

    if outputs["C"]["status"] == "COMPLETE":
        passed = bool(outputs["C"].get("git_path"))
        checks.append(Check("manifest:C-git-path", passed, "git path present" if passed else "C COMPLETE requires git_path"))

    if any(stage["status"] == "BLOCKED" for stage in stages.values()):
        passed = bool(data.get("blocking_issues"))
        checks.append(Check("manifest:blocker-record", passed, "blocking issue recorded" if passed else "BLOCKED stage requires blocking_issues entry"))

    if require_workflow_complete:
        require_archive_complete = True
    if require_academic_complete:
        checks.extend(_academic_completion_checks(data))
    if require_archive_complete:
        checks.extend(_archive_completion_checks(data))
    return checks
...
def extract_comment_section(markdown: str) -> str:
    ...
    for index, line in enumerate(lines):
        if COMMENT_HEADING_PATTERN.fullmatch(line.strip()):
            ...
            break
    if start is None:
        raise ValueError("Comment section not found. Use a level-2 heading such as '## 评论'.")
    end = len(lines)
...
def check_c(root: Path, path: Path | None, required: bool, canonical_abstract: Path | None) -> list[Check]:
    if path is None:
    ...
    missing_fields = [field for field, aliases in C_FIELD_ALIASES.items() if find_section(sections, aliases) is None]
    checks = [Check("artifact:C", not missing_fields, "required sections found" if not missing_fields else f"missing C sections: {', '.join(missing_fields)}")]
    try:
        comment = extract_comment_section(markdown)
        count = effective_chinese_characters(comment)
        minimum = profile["minimum_comment_chars"]
        checks.append(Check("comment_chars", count >= minimum, f"{count}/{minimum} Chinese characters in comment body"))
    ...
    if reviewer_required:
        ...
        checks.append(Check("C:reviewer", passed, "reviewer matches profile" if passed else "reviewer does not match submission_profile"))
    if canonical_abstract is not None:
        try:
            ...
            checks.append(Check("C:canonical-abstract", passed, "canonical Abstract reused exactly (ignoring whitespace)" if passed else "C translated Abstract differs from canonical_abstract.md"))
        except OSError as exc:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Require academic weekly closure; Zotero pending actions are allowed. Requires --manifest.",
    )
    parser.add_argument(
        "--require-archive-complete",
        action="store_true",
        help="Require academic closure plus verified A/B Zotero keys and no pending Zotero actions. Requires --manifest.",
    )
    parser.add_argument(
        "--require-workflow-complete",
        action="store_true",
        help="Legacy alias for --require-archive-complete; requires --manifest.",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    requires_manifest = (
        ...
        or args.require_workflow_complete
    )
    ...
    if args.manifest:
        checks.extend(
            check_manifest(
                ...
                require_academic_complete=args.require_academic_complete,
