...
最终焦点论文已有可验证的 Main PDF 时，优先在目标 Zotero 分类中导入该 PDF，并使用 Zotero 的文献识别功能生成父条目。写入后必须同时验证 DOI/题名身份、目标分类、父条目键及 Main PDF 子附件键。

不得把没有 Main PDF 子附件的元数据-only 父条目当作常规归档完成路径。只有在 PDF 识别不可用或失败时才回退到元数据创建，并明确记录原因；随后仍应尽快把 Main PDF 挂到该父条目并验证。
