...
12. Run Coverage, Semantic and Numeric QC, then run `validate_translation_package.py` to generate validator-owned `numeric_integrity.json`, `untranslated_residual_audit.json`, `typography_fit.json`, `layout_diff.json`, and `translation_validation.json`.
...
`FULL_MIRROR` with `EXACT_TEXT_FRAME` additionally requires schema-v2 `source_inventory.json`, closed `text_frame_inventory.jsonl`, `font_map.json`, `mirror_layout_plan.json`, validator-generated `numeric_integrity.json`, `untranslated_residual_audit.json`, `typography_fit.json`, and `layout_diff.json`, one-to-one pages, exact table cells, embedded SimSun, role-specific 95%-110% frame sizing, semantic Chinese punctuation/citations, zero unapproved visible English runs, and zero rendered changes outside reviewed replacement regions. Do not apply these exact-layout requirements to `MAIN_ONLY` or `ABSTRACT_ONLY`.
